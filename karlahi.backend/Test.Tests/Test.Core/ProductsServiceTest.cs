...
namespace Test.Tests
{
    public class ProductsServicesTest : IClassFixture<ProductServiceFixture>
    {
        ...
        {
            var res = await _productService.ProductsService.GetBySpecAsync(new ProductByIdSpecification(1));
            Assert.NotNull(res);
        }
        ...
        {
            int id = 15;
            await _productService.ProductsService.RemoveBySpecAsync(new ProductByIdSpecification(id));
            var res = await _productService.ProductsService.GetBySpecAsync(new ProductByIdSpecification(id));
            Assert.Null(res);
        }
        ...
        public async Task UpdateAsync()
        {
            ...
            var res2 = await _productService.ProductsService.GetBySpecAsync(new ProductByIdSpecification(id));
